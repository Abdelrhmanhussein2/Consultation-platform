...
export function applyFilters(consultants, filters) {
  const { search = '', selSpecs = [], selComms = [], cityF = '', availF = false, maxPrice = null, minRat = '' } = filters;
  ...
  return consultants.filter(c => {
    ...
    // ── 2. Specialization checkbox (match by ID or name) ───
    if (selSpecs.length > 0) {
      const cSpecId = String(c.main_specialization_id || c.specialization_id || '');
      const cSpecName = c.specialization_name || '';
      const matched = selSpecs.some(s => s === cSpecId || s === cSpecName);
    ...
    // ── 6. Price Slider Filter ─────────────────────────────────────
    if (maxPrice !== null && maxPrice !== undefined && Number(maxPrice) > 0 && Number(maxPrice) < 250) {
      const p = c.price ?? c.price_per_hour ?? 0;
      const numP = typeof p === 'number' ? p : (parseFloat(p) || 0);
      if (numP > Number(maxPrice)) return false;
    }

    // ── 7. Rating Filter ───────────────────────────────────────────
    if (minRat) {
      const r = c.average_rating ?? c.rating ?? 0;
      const numR = typeof r === 'number' ? r : (parseFloat(r) || 0);
      if (numR < parseFloat(minRat)) return false;
    }
